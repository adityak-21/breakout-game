...
let scoreValue, ball, brickWall, paddle, collision, ydir, xdir, getYDir, getXDir;
...
describe('Collision Tests with JSDOM', () => {
    ...
    before(async() => {
        ...
        ({collision, ydir, xdir, getYDir, getXDir} = await import('../scripts/collision.js'));
    ...
    it('should reverse ydir when ball hits the top boundary', () => {
        ...
        collision(paddle.getBoundingClientRect(), left, right, up, down, check, check0, 0, 0 ,0, 0);
    ...
    it('should reverse xdir when ball hits the left boundary', () => {
        ...
        collision(paddle.getBoundingClientRect(), left, right, up, down, check, 0, check1, 0 ,0, 0);
        expect(getXDir()).to.equal(-1.0);
    });

    // it('should restart when ball hits the bottom boundary', () => {
    //     ball.getBottom = () => -1;
    //     const paddleRect = {
    //         left: 0,
    //         right: 0,
    ...
    //     sinon.stub(paddle, 'getBoundingClientRect').returns(paddleRect);
    //     left=0, right=0, up=0, down=0, check=0, check3=1;
    //     collision(paddle.getBoundingClientRect(), left, right, up, down, check, 0, 0, 0 ,check3, 0);
    //     expect(ball.getLeft()).to.equal(10);
    // });
    ...
    //     ball.getRight = () => 200;
    //     collision(paddle.getBoundingClientRect());
    //     expect(xdir).to.equal(-1.0);
    // });

    // it('should increase score when ball hits the paddle', () => {
    //     const paddleRect = {
    //         getTop: () => 90,
    //         getBottom: () => 100,
    //         getLeft: () => 50,
    ...
    //     sinon.stub(paddle, 'getBoundingClientRect').returns(paddleRect);
    //     collision(paddle.getBoundingClientRect());
    //     expect(parseInt(scoreValue.innerHTML)).to.equal(1);
    // });
    // it('should destroy brick when ball hits a brick', () => {
    //     const brick = {
    //         getTop: () => 90,
    //         getBottom: () => 100,
    //         getLeft: () => 50,
    ...
    //     const destroySpy = sinon.spy(brick, 'destroy');
    //     collision(paddle.getBoundingClientRect());
